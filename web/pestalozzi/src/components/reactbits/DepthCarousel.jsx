// De reactbits.dev/components/depth-carousel — copiado tal cual salvo
// el import de gsap, que va por _gsapSetup.js: cada isla de Astro se
// hidrata por separado y traía su propia copia del módulo.
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

import { gsap } from './_gsapSetup.js';
...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    ro.observe(root);
    // Medición inmediata: si el observer tarda en disparar, las tarjetas
    // quedan sin posicionar. Mismo motivo que en el masonry anterior.
    const w = root.getBoundingClientRect().width;
    if (w > 0) {
      const cfg = cfgRef.current;
      scaleRef.current = clamp(w / (cfg.cardWidth + Math.abs(cfg.spread) * 2 + 120), 0.4, 1);
      layout(posRef.current);
    }
  ...
  return (
    <div
      ...
      aria-label="Fotografías del colegio"
      tabIndex={0}
    ...
    >
      ...
      {showControls && count > 1 && (
        <>
          <button type="button" className="depth-carousel__arrow depth-carousel__arrow--prev" aria-label="Foto anterior" onClick={() => navigateBy(-1)}>
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
              <path d="M15 5l-7 7 7 7" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button type="button" className="depth-carousel__arrow depth-carousel__arrow--next" aria-label="Foto siguiente" onClick={() => navigateBy(1)}>
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel__dots" role="tablist" aria-label="Fotos">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
