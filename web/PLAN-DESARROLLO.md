...
A la fecha, `pestalozzi/` quedó a medio camino entre dos rumbos:

- `_vanilla-respaldo/` — los 4 HTML, apuntando a rutas `css/` e `img/` que ya no existen ahí
- `src/styles/` — los 4 CSS
- `public/img/` — las imágenes optimizadas a WebP
- `package.json` + `astro.config.mjs` — andamiaje de Astro, sin páginas ni `node_modules`
- `js/main.js` — **se perdió**

**Ninguna de las dos versiones funciona hoy.** Antes de escribir más código hay que
decidir el rumbo: vanilla (lo que dice este plan) o Astro (build step, componentes,
sitemap automático).
